...
$query = 'SELECT article.id, article.name, article.idUser, article.textOfArticle, article.idCategorie, article.image_filename FROM article WHERE article.id = :idArticle';
...
$query = 'SELECT categorie.id, categorie.name FROM categorie';
$statement = $dbConnection->prepare($query);
$successOrFailure = $statement->execute();
